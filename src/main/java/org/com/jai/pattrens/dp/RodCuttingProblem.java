...
public class RodCuttingProblem {
    ...
    public static int getMaxProfit(int[] pieces, int[] profits, int rodLength) {


        if (pieces.length == 0 || profits.length == 0 || rodLength <= 0) {
        ...
        for (int i = 0; i < numberOfPieces; i++) {
            ...
            for (int j = 1; j <= rodLength; j++) {
                ...
                if (pieces[i] <= j) {
                    ...
                    includingCurrentPieceProfit = profits[i] + arr[i][j - pieces[i]];
                }
